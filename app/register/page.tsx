...
import AvailabilityEditor from '@/components/site/availability-editor'
...
export default function RegisterSection() {
    ...
    const [clients, setClients] = useState<Client[]>([
        { id: 'cl_1', name: 'Sita Shrestha', coins: 100 },
        { id: 'cl_2', name: 'Ram Khatri', coins: 50 },
    ])
    ...
    const [professionals, setProfessionals] = useState<Professional[]>([
        {
            ...
            bio: 'Experienced therapist specializing in stress management.',
    ...
    const handleClientRegister = (name: string) => {
        ...
        const newClient = { id, name, coins: 0 }
        setClients((prev) => [newClient, ...prev])
        setCurrentClientId(id)
        toast({ title: `Client ${name} registered` })
    }
    ...
    const handleProRegister = (p: Omit<Professional, 'id'>) => {
        ...
        toast({ title: `Professional ${p.name} registered` })
    }
                ...
                <div className="grid sm:grid-cols-[1fr_auto] gap-2 items-end">
                    <div>
                        ...
                        <Select value={currentClientId ?? ''} onValueChange={(v) => onSwitch(v || null)}>
                            <SelectTrigger id="switch-client">
                                <SelectValue placeholder="Select client" />
                            </SelectTrigger>
                            <SelectContent>
                                {existing.map((c) => (
                                    <SelectItem key={c.id} value={c.id}>
                                        {c.name} ({c.coins} coins)
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                    <Button variant="outline" className="transition-colors" onClick={() => onSwitch(null)}>
                        Sign out
                    </Button>
                </div>
            )}
...
function ProfessionalForm({
    ...
    onRegister: (p: Omit<Professional, 'id'>) => void
    ...
    const toggleMethod = (m: CommMethod) =>
    ...
    return (
        <div className="space-y-4">
            <div className="grid md:grid-cols-3 gap-3">
                ...
                <div className="md:col-span-1">
                    ...
                    <Input
                        ...
                        placeholder="e.g. Therapist, Tutor, Lawyer"
                    />
                </div>
                <div className="md:col-span-1">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                    <button
                        ...
                        className={cn(
                            ...
                            methods.includes('chat') && 'bg-teal-600 text-white border-transparent'
                        )}
                    >
                    ...
                    <button
                        type="button"
                        onClick={() => toggleMethod('audio')}
                        className={cn(
                            'px-3 py-2 text-sm rounded-md border transition-colors',
                            methods.includes('audio') && 'bg-teal-600 text-white border-transparent'
                        )}
                    >
                    ...
                    <button
                        type="button"
                        onClick={() => toggleMethod('video')}
                        className={cn(
                            'px-3 py-2 text-sm rounded-md border transition-colors',
                            methods.includes('video') && 'bg-teal-600 text-white border-transparent'
                        )}
                    >
            ...
            <AvailabilityEditor value={defaultAvailability} onChange={() => { }} />
